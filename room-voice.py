...
def apply_reverb_simulation(audio, sample_rate, decay_time):
    # decay_time not currently used directly, but kept for API compatibility
...
def create_single_word_voice(word, sample_rate, duration, word_index,
    ...
    # Low‑pass filter to smooth formants
    ...
    beep_envelope[decay_start_sample:] = np.linspace(
        1, 0.05, len(beep_envelope[decay_start_sample:])
    )
    beep *= beep_envelope
    ...
    fade_in = min(fade_in, len(final))
    fade_out = min(fade_out, len(final))
    if fade_in > 0:
    ...
    # Place word into overall buffer, with bounds check
    output = np.zeros(total_length, dtype=np.float32)
    start_i = int(start_time * sample_rate)
    if start_i >= total_length:
        return output  # word would start after end; ignore

    end_i = start_i + len(final)
    if end_i > total_length:
        # Trim final so it fits in the output buffer
        trim_len = total_length - start_i
        if trim_len <= 0:
            return output
        final = final[:trim_len]
        end_i = total_length
...
background = create_continuous_bg(
    total_time, sample_rate,
...
for i, (word, dur) in enumerate(zip(words, word_durations)):
    overlay = create_single_word_voice(
        word, sample_rate, dur, i,
